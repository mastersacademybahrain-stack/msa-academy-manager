...
export default async function(req,res){
 ...
 const base='https://msa-academy-manager.hatchable.site/api/evaluation-report-render-v291?token='+encodeURIComponent(token)+'&v=296&design=option2-exact';
 try{const bytes=await browser.pdf(base,{width:'210mm',height:'297mm',printBackground:true});res.setHeader('X-MSA-PDF-Renderer','v296-option2-exact');res.setHeader('content-type','application/pdf');res.setHeader('content-disposition','inline; filename="MSA_Player_Evaluation_Report.pdf"');return res.send(bytes)}finally{await db.query('DELETE FROM report_pdf_tokens WHERE token=$1',[token])}
}
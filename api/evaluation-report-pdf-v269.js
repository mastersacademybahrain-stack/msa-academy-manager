...
export default async function(req,res){
 ...
 const base='https://msa-academy-manager.hatchable.site/api/evaluation-report-render-v269?token='+encodeURIComponent(token)+'&v=269';
 try{
  const bytes=await browser.pdf(base,{width:'200mm',height:'300mm',printBackground:true});
  res.setHeader('X-MSA-PDF-Renderer','v269');
  res.setHeader('content-type','application/pdf');
  res.setHeader('content-disposition','attachment; filename="MSA_Player_Evaluation_Report_FINAL_269.pdf"');

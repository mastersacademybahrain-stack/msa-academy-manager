...
export default async function(req,res){
 ...
 const base='https://msa-academy-manager.hatchable.site/api/evaluation-report-render-v272?token='+encodeURIComponent(token)+'&v=272';
 try{const bytes=await browser.pdf(base,{width:'200mm',height:'300mm',printBackground:true});res.setHeader('X-MSA-PDF-Renderer','v272-premium');res.setHeader('content-type','application/pdf');res.setHeader('content-disposition','inline; filename="MSA_Player_Evaluation_Report_PREMIUM_272.pdf"');return res.send(bytes)}finally{await db.query('DELETE FROM report_pdf_tokens WHERE token=$1',[token])}
}
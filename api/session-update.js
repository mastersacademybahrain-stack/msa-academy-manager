...
export default async function(req,res){
 const {session_id,sport,day_of_week,start_time,coach_id=null,coach_ids=[],effective_start_date,weeks,tennis_categories=[],location=null}=req.body||{};
 ...
 const x=await db.query('INSERT INTO academy_sessions(sport,day_of_week,start_time,coach_id,start_date,end_date,tennis_categories,location) VALUES($1,$2,$3,$4,$5,$6,$7,$8) RETURNING id,sport,day_of_week,start_time,coach_id,start_date,end_date,tennis_categories,location',[sport,+day_of_week,start_time,primaryCoach,effective_start_date,edISO,cats,location]);
 ...
 if(oldEnd && oldEnd>ed){
  ...
  const sc=await db.query('SELECT coach_id FROM session_coaches WHERE session_id=$1',[session_id]);
  for(const r of sc.rows)await db.query('INSERT INTO session_coaches(session_id,coach_id) VALUES($1,$2) ON CONFLICT DO NOTHING',[c.rows[0].id,r.coach_id]);
 }
